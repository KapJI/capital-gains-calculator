...
class CapitalGainsCalculator:
    ...
    def convert_to_hmrc_transactions(
        ...
        dividends: dict[tuple[str, datetime.date], ForeignCurrencyAmount] = defaultdict(
            ForeignCurrencyAmount
        )
        dividends_tax: dict[tuple[str, datetime.date], ForeignCurrencyAmount] = (
            defaultdict(ForeignCurrencyAmount)
        )
        interest_uk = Decimal(0)
        ...
        for i, transaction in enumerate(transactions):
            ...
            if transaction.action is ActionType.TRANSFER:
                ...
                assert symbol is not None
                new_balance += amount
                dividends[(symbol, transaction.date)] += ForeignCurrencyAmount(
                    amount, currency
                )
                ...
                assert symbol is not None
                new_balance += amount
                dividends_tax[(symbol, transaction.date)] += ForeignCurrencyAmount(
                    amount, currency
                )
                ...
                if self.date_in_tax_year(transaction.date):
                    if transaction.currency == COUNTRY_CURRENCY:
                        ...
                        amount = self.converter.to_gbp_for(amount, transaction)
                        interest_foreign += amount
        ...
        sum_dividends, sum_dividends_tax, sum_div_interests = self.process_dividends(
            dividends, dividends_tax
        )

        self.total_foreign_interest = interest_foreign + sum_div_interests
        self.total_uk_interest = interest_uk
    ...
    def process_dividends(
        self,
        dividends: dict[tuple[str, datetime.date], ForeignCurrencyAmount],
        dividends_taxes: dict[tuple[str, datetime.date], ForeignCurrencyAmount],
    ) -> tuple[Decimal, Decimal, Decimal]:
        """Process all dividends events and taxes.

        It produces the totals for the current tax year.
        """
        sum_dividends, sum_div_taxes, sum_interests = Decimal(0), Decimal(0), Decimal(0)
        for (symbol, date), foreign_amount in dividends.items():
            tax = dividends_taxes[(symbol, date)]
            ...
            if self.date_in_tax_year(date):
                ...
                if is_interest_fund:
                    sum_interests += amount
                else:
                    sum_dividends += amount
                sum_div_taxes += tax_amount

        return sum_dividends, sum_div_taxes, sum_interests
    ...
    def calculate_capital_gain(
        ...
        return CapitalGainsReport(
            ...
            self.total_uk_interest,
            self.total_foreign_interest,
